"""Artifact key and event flow for libreoffice-worker docx_export."""
...
class DocxExportWorkerCommand:
    ...
    @classmethod
    def from_message(cls, message: dict[str, object]) -> "DocxExportWorkerCommand":
        command = cls(
    ...
    def validate(self) -> None:
        if self.input_type not in {"pdf", "docx"}:
            raise ValueError(f"libreoffice-worker docx_export requires input_type 'pdf' or 'docx', got {self.input_type!r}")
        if self.stage != "docx_export":
            raise ValueError(f"libreoffice-worker requires stage='docx_export', got {self.stage!r}")
        if not self.object_prefix:
            raise ValueError("object_prefix is required")


@dataclass(frozen=True)
class DocxExportArtifactKeys:
    translated_docx: str
    final_docx: str
    final_pdf: str
...
def artifact_keys_for(command: DocxExportWorkerCommand) -> DocxExportArtifactKeys:
    prefix = command.object_prefix
    return DocxExportArtifactKeys(
        translated_docx=command.input_object_key or f"{prefix}/04_replace/translated.docx",
...
def process_docx_export_command(
    ...
    *,
    ...
    command = DocxExportWorkerCommand.from_message(message)
    keys = artifact_keys_for(command)
    work_dir = work_root / _safe_segment(command.job_id)
    input_docx_path = work_dir / "translated.docx"
    final_docx_path = work_dir / "final.docx"
    final_pdf_path = work_dir / "final.pdf"

    store.download_file(keys.translated_docx, input_docx_path)
    export_docx_artifacts(
...
def stage_completed_event(command: DocxExportWorkerCommand, outputs: dict[str, str]) -> dict[str, object]:
    return {
...
def stage_failed_event(message: dict[str, object], error: Exception) -> dict[str, object]:
    ...
    stage = str(message.get("stage", "docx_export"))
    return {
        "event_type": "stage.failed",
        "job_id": job_id,
        "input_type": input_type,
        "stage": stage,
        "error_code": "DOCX_EXPORT_WORKER_FAILED",
