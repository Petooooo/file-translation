...
from libreoffice_worker.artifacts import event_queue_key, process_docx_export_command, stage_failed_event
from libreoffice_worker.export import VALID_PDF_MODES, export_docx_artifacts


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="libreoffice-worker docx_export runtime")
    ...
    config = load_config("libreoffice-worker", "worker", "docx_export")
    if args.smoke:
        print_smoke(config)
        return 0
    ...
    logger.info("starting libreoffice-worker skeleton stage=docx_export")
    logger.info("command queue=%s", config.command_queues["docx_export"])
    ...
    while not stop["requested"]:
        ...
        logger.info("worker idle heartbeat stage=docx_export")
