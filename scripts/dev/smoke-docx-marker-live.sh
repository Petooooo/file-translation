...
NETWORK="${NETWORK:-ft-docx-marker-live}"
MINIO_CONTAINER="${MINIO_CONTAINER:-ft-marker-minio-live}"
RABBITMQ_CONTAINER="${RABBITMQ_CONTAINER:-ft-marker-rabbitmq-live}"
WORKER_CONTAINER="${WORKER_CONTAINER:-ft-docx-marker-worker-live}"
SEED_CONTAINER="${SEED_CONTAINER:-ft-docx-marker-seed-live}"
PUBLISH_CONTAINER="${PUBLISH_CONTAINER:-ft-docx-marker-publish-live}"
...
OBJECT_PREFIX="${OBJECT_PREFIX:-2026-01-21/12345678/markersmoke1}"
JOB_ID="${JOB_ID:-live-docx-marker-smoke}"
OUT_DIR="${OUT_DIR:-out/docx-marker-live}"
KEEP_LIVE_SMOKE="${KEEP_LIVE_SMOKE:-0}"

COMMAND_QUEUE="${COMMAND_QUEUE:-q.commands.docx_marker}"
...
usage() {
  cat <<'EOF'
Usage: scripts/dev/smoke-docx-marker-live.sh

Runs a Docker-based live smoke for:
  RabbitMQ command -> libreoffice-worker docx_marker -> MinIO marker DOCX -> RabbitMQ event

Environment variables:
  NETWORK                 Docker network. Default: ft-docx-marker-live
  ...
  OBJECT_PREFIX           Default: 2026-01-21/12345678/markersmoke1
  JOB_ID                  Default: live-docx-marker-smoke
  OUT_DIR                 Default: out/docx-marker-live
...
cleanup() {
  status=$?
  if [ "$status" -ne 0 ]; then
    docker logs "$WORKER_CONTAINER" >/tmp/docx-marker-worker-live.log 2>&1 || true
    if [ -s /tmp/docx-marker-worker-live.log ]; then
      printf '[INFO] worker logs:\n' >&2
      cat /tmp/docx-marker-worker-live.log >&2
    fi
  fi
...
note "Creating sample final DOCX"
rm -rf "$OUT_DIR"
mkdir -p "$OUT_DIR"
docker run --rm -i \
...
with zipfile.ZipFile(out / "final.docx", "w") as archive:
    archive.writestr("word/document.xml", document_xml)
PY

note "Waiting for MinIO/RabbitMQ and seeding final DOCX"
docker run --rm -i \
  --name "$SEED_CONTAINER" \
  --network "$NETWORK" \
...
note "Starting libreoffice-worker docx_marker consumer"
docker run -d \
  ...
  -e "RABBITMQ_PASSWORD=${RABBITMQ_PASSWORD}" \
  "$WORKER_IMAGE" \
  python /app/service/worker.py --consume-marker >/dev/null

note "Publishing docx_marker command and waiting for completed event"
docker run --rm -i \
...
if not completed:
    raise SystemExit("timed out waiting for docx_marker stage.completed event")
...
out = Path("/work/out")
marker_docx = out / "marker.docx"
client.fget_object(bucket, expected_outputs["marker_docx"], str(marker_docx))

with zipfile.ZipFile(marker_docx, "r") as archive:
    root = ET.fromstring(archive.read("word/document.xml"))
texts = [
    node.text or ""
    for node in root.iter("{http://schemas.openxmlformats.org/wordprocessingml/2006/main}t")
]
if texts != ["[ko]¡Hello¡world", "NoSpace", "A¡B¡C"]:
    raise SystemExit(f"unexpected marker DOCX texts: {texts!r}")
PY

pass "docx_marker live smoke completed"
