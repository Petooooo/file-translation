...
NETWORK="${NETWORK:-ft-pdf-route-e2e}"
MINIO_CONTAINER="${MINIO_CONTAINER:-ft-pdf-route-e2e-minio}"
RABBITMQ_CONTAINER="${RABBITMQ_CONTAINER:-ft-pdf-route-e2e-rabbitmq}"
POSTGRES_CONTAINER="${POSTGRES_CONTAINER:-ft-pdf-route-e2e-postgres}"
JOB_SERVICE_CONTAINER="${JOB_SERVICE_CONTAINER:-ft-pdf-route-e2e-job-service}"
PDF2DOCX_CONTAINER="${PDF2DOCX_CONTAINER:-ft-pdf-route-e2e-pdf2docx}"
PDF_EXTRACT_CONTAINER="${PDF_EXTRACT_CONTAINER:-ft-pdf-route-e2e-extract}"
TRANSLATE_CONTAINER="${TRANSLATE_CONTAINER:-ft-pdf-route-e2e-translate}"
PDF_REPLACE_CONTAINER="${PDF_REPLACE_CONTAINER:-ft-pdf-route-e2e-replace}"
PDF_EXPORT_CONTAINER="${PDF_EXPORT_CONTAINER:-ft-pdf-route-e2e-export}"
PDF_MARKER_CONTAINER="${PDF_MARKER_CONTAINER:-ft-pdf-route-e2e-marker}"
PDF2HWPX_CONTAINER="${PDF2HWPX_CONTAINER:-ft-pdf-route-e2e-pdf2hwpx}"
EMAIL_CONTAINER="${EMAIL_CONTAINER:-ft-pdf-route-e2e-email}"
SEED_CONTAINER="${SEED_CONTAINER:-ft-pdf-route-e2e-seed}"
CANCEL_DRIVER_CONTAINER="${CANCEL_DRIVER_CONTAINER:-ft-pdf-route-e2e-cancel}"
SETUP_DRIVER_CONTAINER="${SETUP_DRIVER_CONTAINER:-ft-pdf-route-e2e-setup}"
VERIFY_DRIVER_CONTAINER="${VERIFY_DRIVER_CONTAINER:-ft-pdf-route-e2e-verify}"
DB_CHECK_CONTAINER="${DB_CHECK_CONTAINER:-ft-pdf-route-e2e-db-check}"
...
PDF2DOCX_WORKER_IMAGE="${PDF2DOCX_WORKER_IMAGE:-${DOCKER_NAMESPACE}/file-translation-pdf2docx-worker:${IMAGE_TAG}}"
PDF2DOCX_BASE_IMAGE="${PDF2DOCX_BASE_IMAGE:-petoo/pdf2docx:0.5.13-py311-static}"
PDF_EXTRACT_WORKER_IMAGE="${PDF_EXTRACT_WORKER_IMAGE:-${DOCKER_NAMESPACE}/file-translation-docx-extract-worker:${IMAGE_TAG}}"
TRANSLATE_WORKER_IMAGE="${TRANSLATE_WORKER_IMAGE:-${DOCKER_NAMESPACE}/file-translation-translate-worker:${IMAGE_TAG}}"
PDF_REPLACE_WORKER_IMAGE="${PDF_REPLACE_WORKER_IMAGE:-${DOCKER_NAMESPACE}/file-translation-docx-replace-worker:${IMAGE_TAG}}"
...
OUT_DIR="${OUT_DIR:-out/pdf-route-e2e}"
USER_ID="${USER_ID:-12345678}"
MAIN_FILE_ID="${MAIN_FILE_ID:-pdfroutee2e}"
MID_CANCEL_FILE_ID="${MID_CANCEL_FILE_ID:-pdfroutecancelmid}"
EMAIL_CANCEL_FILE_ID="${EMAIL_CANCEL_FILE_ID:-pdfroutecancelemail}"
MAIN_INPUT_KEY="${MAIN_INPUT_KEY:-2026-01-21/${USER_ID}/${MAIN_FILE_ID}/input/original.pdf}"
MID_CANCEL_INPUT_KEY="${MID_CANCEL_INPUT_KEY:-2026-01-21/${USER_ID}/${MID_CANCEL_FILE_ID}/input/original.pdf}"
EMAIL_CANCEL_INPUT_KEY="${EMAIL_CANCEL_INPUT_KEY:-2026-01-21/${USER_ID}/${EMAIL_CANCEL_FILE_ID}/input/original.pdf}"
PDF_MARKER_TOKEN="${PDF_MARKER_TOKEN:-¡}"
...
usage() {
  cat <<'EOF'
Usage: scripts/dev/smoke-pdf-route-e2e.sh

Runs a Docker-based route-level PDF E2E smoke:
  job-service create PDF job -> pdf2docx -> docx_extract ->
  docx_translate -> docx_replace -> docx_export -> docx_marker ->
  pdf2hwpx -> email_send -> completed

This smoke uses the custom static anchored pdf2docx worker first, then
placeholder DOCX PDF export and placeholder pdf2hwpx output. It does not
enable real LibreOffice export, real pdf2hwpx conversion, real email delivery,
or Helm.
EOF
}
...
cleanup() {
  ...
  if [ "$status" -ne 0 ]; then
    for container in \
      ...
      "$PDF2DOCX_CONTAINER" \
      "$PDF_EXTRACT_CONTAINER" \
      "$TRANSLATE_CONTAINER" \
      "$PDF_REPLACE_CONTAINER" \
      "$PDF_EXPORT_CONTAINER" \
      "$PDF_MARKER_CONTAINER" \
      "$PDF2HWPX_CONTAINER" \
      "$EMAIL_CONTAINER"; do
...
docker run --rm \
  ...
  "$PDF2DOCX_BASE_IMAGE" \
  python /opt/pdf2docx/examples/static_anchored_smoke.py --out-dir /work/out --with-report >/tmp/pdf-route-e2e-base-smoke.log

note "Waiting for MinIO/RabbitMQ, creating bucket, and seeding fixed PDF input keys"
docker run --rm -i \
  --name "$SEED_CONTAINER" \
  --network "$NETWORK" \
...
sample = Path("/work/out/sample.pdf")
...
def publish_completed(channel, *, job_id: str, stage: str, object_prefix: str) -> None:
    outputs_by_stage = {
        "pdf2docx": {
            "converted_docx": f"{object_prefix}/01_pdf2docx/converted.docx",
            "pdf2docx_report_json": f"{object_prefix}/reports/pdf2docx.report.json",
            "pdf2docx_report_md": f"{object_prefix}/reports/pdf2docx.report.md",
        },
...
def create_pdf_job(file_id: str, input_key: str) -> dict[str, object]:
    created = http_json(
        "POST",
        "/jobs",
        {
...
mid_job = create_pdf_job(os.environ["MID_CANCEL_FILE_ID"], os.environ["MID_CANCEL_INPUT_KEY"])
mid_job_id = str(mid_job["job_id"])
mid_prefix = str(mid_job["object_prefix"]).strip("/")
http_json("POST", f"/jobs/{mid_job_id}/cancel")
publish_completed(channel, job_id=mid_job_id, stage="pdf2docx", object_prefix=mid_prefix)
wait_job(mid_job_id, "cancelled", "cancelled")

unexpected_extract: list[dict[str, object]] = []
deadline = time.time() + 5
while time.time() < deadline:
    method, _props, body = channel.basic_get("q.commands.docx_extract", auto_ack=True)
    if method is None:
        time.sleep(0.2)
        continue
    decoded = json.loads(body.decode("utf-8"))
    if isinstance(decoded, dict):
        unexpected_extract.append(decoded)
if unexpected_extract:
    raise AssertionError(f"mid-route cancelled job published docx_extract: {unexpected_extract}")
purge_all(channel)

email_job = create_pdf_job(os.environ["EMAIL_CANCEL_FILE_ID"], os.environ["EMAIL_CANCEL_INPUT_KEY"])
email_job_id = str(email_job["job_id"])
email_prefix = str(email_job["object_prefix"]).strip("/")
for stage in ["pdf2docx", "docx_extract", "docx_translate", "docx_replace", "docx_export", "docx_marker", "pdf2hwpx"]:
...
note "Creating PDF E2E job and pre-seeding dynamic object_prefix input"
docker run --rm -i \
...
created = http_json(
    ...
    "/jobs",
    {
        ...
        "source_lang": "en",
        "target_lang": "ko",
        "original_filename": "route-e2e.pdf",
...
dynamic_input_key = f"{object_prefix}/input/original.pdf"
client = Minio(
    os.environ["MINIO_ENDPOINT"],
    access_key=os.environ["MINIO_ACCESS_KEY"],
    secret_key=os.environ["MINIO_SECRET_KEY"],
...
note "Starting custom static anchored pdf2docx worker"
docker run -d \
  --name "$PDF2DOCX_CONTAINER" \
  --network "$NETWORK" \
  ...
  -e "RABBITMQ_PASSWORD=${RABBITMQ_PASSWORD}" \
  "$PDF2DOCX_WORKER_IMAGE" \
  python /app/service/worker.py --consume --work-dir /tmp/file-translation/pdf2docx >/dev/null

note "Starting DOCX extract worker for PDF route"
docker run -d \
  --name "$PDF_EXTRACT_CONTAINER" \
  --network "$NETWORK" \
  -e "MINIO_ENDPOINT=http://minio:9000" \
  -e "MINIO_BUCKET=${MINIO_BUCKET}" \
  -e "MINIO_ACCESS_KEY=${MINIO_ACCESS_KEY}" \
...
note "Starting DOCX replace worker for PDF route"
docker run -d \
  --name "$PDF_REPLACE_CONTAINER" \
  --network "$NETWORK" \
  -e "MINIO_ENDPOINT=http://minio:9000" \
  -e "MINIO_BUCKET=${MINIO_BUCKET}" \
  -e "MINIO_ACCESS_KEY=${MINIO_ACCESS_KEY}" \
  ...
  -e "RABBITMQ_USERNAME=${RABBITMQ_USERNAME}" \
  -e "RABBITMQ_PASSWORD=${RABBITMQ_PASSWORD}" \
  "$PDF_REPLACE_WORKER_IMAGE" \
  python /app/service/worker.py --consume --work-dir /tmp/file-translation/docx-replace >/dev/null

note "Starting DOCX export worker for PDF route"
docker run -d \
  --name "$PDF_EXPORT_CONTAINER" \
  --network "$NETWORK" \
...
note "Starting DOCX marker worker for PDF route"
docker run -d \
  --name "$PDF_MARKER_CONTAINER" \
  --network "$NETWORK" \
  -e "DOCX_MARKER_TOKEN=${PDF_MARKER_TOKEN}" \
  -e "MINIO_ENDPOINT=http://minio:9000" \
  -e "MINIO_BUCKET=${MINIO_BUCKET}" \
  -e "MINIO_ACCESS_KEY=${MINIO_ACCESS_KEY}" \
  -e "MINIO_SECRET_KEY=${MINIO_SECRET_KEY}" \
...
note "Waiting for PDF route E2E completion and validating artifacts"
docker run --rm -i \
  ...
  -e "RABBITMQ_USERNAME=${RABBITMQ_USERNAME}" \
  -e "RABBITMQ_PASSWORD=${RABBITMQ_PASSWORD}" \
  -e "PDF_MARKER_TOKEN=${PDF_MARKER_TOKEN}" \
  -v "$PWD/$OUT_DIR:/work/out" \
  "$EMAIL_WORKER_IMAGE" \
  python - <<'PY'
import io
...
bucket = os.environ["MINIO_BUCKET"]
marker_token = os.environ["PDF_MARKER_TOKEN"]
...
if text_units.get("input_type") != "pdf" or len(text_units.get("units", [])) < 2:
...
expected_translations = [
    "[ko] Docker smoke body paragraph page 1.",
    "[ko] This body text must remain in the DOCX body.",
]
if not all(value in translated_values for value in expected_translations):
    raise AssertionError(f"translated units missing mock translations: {translated_values}")

converted_texts = docx_texts(expected_keys["converted_docx"])
if "Docker smoke body paragraph page 1." not in converted_texts:
    raise AssertionError(f"converted DOCX missing static anchored sample text: {converted_texts}")
report_json = json.loads(get_object(expected_keys["pdf2docx_report_json"]).decode("utf-8"))
if report_json.get("status") != "converted":
    raise AssertionError(f"unexpected pdf2docx JSON report: {report_json}")
report_markdown = get_object(expected_keys["pdf2docx_report_md"]).decode("utf-8")
if "static" not in report_markdown.lower() and "anchored" not in report_markdown.lower():
    raise AssertionError("pdf2docx markdown report did not mention static/anchored conversion")

translated_texts = docx_texts(expected_keys["translated_docx"])
if not all(value in translated_texts for value in expected_translations):
...
if metadata.get("input_type") != "pdf" or metadata.get("job_id") != job_id:
...
if payload.get("status") != "completed" or payload.get("current_stage") != "completed":
    raise AssertionError(f"main PDF E2E job did not complete in PostgreSQL: {payload}")
for stage in [
    "pdf2docx",
    "docx_extract",
    "docx_translate",
...
if payload.get("translated_hwpx_key") is not None:
    raise AssertionError(f"PDF route should not set translated_hwpx_key: {payload.get('translated_hwpx_key')}")
