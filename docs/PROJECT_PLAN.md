...
| 14. Closed-Network Rehearsal and Airgap Bundle | MVP completed | Prove external dependency mode locally and package transfer artifacts for closed-network import. | Closed rehearsal install/smoke validates external PostgreSQL/RabbitMQ/MinIO, existing Secret, queue init, bucket init, health/admin endpoints, and HWPX route E2E; airgap scripts build image tar files, packaged Helm chart, closed example values, placeholder Secret helper, manifest, checksums, and install-order README. |
